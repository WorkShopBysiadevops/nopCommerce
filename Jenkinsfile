pipeline{
  ...
  stages{
    ...
    stage(infraprovisioning){
     steps{
      sh(script: 'cd infra/Terraform && pwd && terraform init && terraform workspace select dev && terraform apply -lock=false -var-file="dev.tfvars" -var "build_number=${BUILD_ID}" -auto-approve')
     }    
    }
    stage('k8s deploy'){
      steps{
